...
annotate( #adding annotation for the marathon
    ...
    x = 25,
    y = 4.3,
    label = "Lake Tahoe Marathon Day, 26.2 miles",
    size = 3,
    color = "black",
    hjust = "inward"
  )  +
  ...
  annotate( #adding annotation for the teton crest trail (outlier)
    ...
    x = 35.9,
    y = 9,
    label = "The Teton Crest Trail was my longest race, clocking in at 38 miles.",
    size = 3,
    color = "black",
    hjust = "inward"
  )  +
...
```{r message = FALSE, warning = FALSE}
#Creating barplot ---
weekly_distribution <- ggplot(total_by_training_week, aes(x = as.numeric(training_week), y = total_miles_by_week, fill = str_replace(sport_type, "TrailRun", "Trail Run"))) + #updating trail run to have a space
  geom_col() + #defining a bar plot
scale_fill_paletteer_d("nord::silver_mine") + #specifying the color scheme
  labs(x = "Training Week", #adding labels to graph
       y = "Total Distance (in miles)", fill = "Type of Activity", title = "Evaluating Total Weekly Training Peaks for Marathon Training", subtitle = "Utilizing Strava data to showcase mix of cardio activities and transition to primarily running within a 6 month training period.") +
...
#Testing ---
weekly_distribution2 <- ggplot(total_by_training_week, aes(x = fct_rev(as.factor(training_week)), y = total_miles_by_week, fill = str_replace(sport_type, "TrailRun", "Trail Run"))) + #updating trail run to have a space
  geom_col() + #defining a bar plot
...
Compiling my Plots Together

```{r}
# Defining my arrangement of plots and storing to referenceable object  ----
final_plot <- weekly_distribution / training_calendar 
final_plot
  # defining the relative heights of rows of arranged plots----
  plot_layout(heights = c(3, 0.5, 1)) + 
  
  ## adding a title, caption, and subtitle (with formatting) to final plot----
  plot_annotation( 
  ...
  # setting margins and font/size/color/margin/justification for title, subtitle, caption--
  theme = theme( 
...
ragg::agg_png(here::here("assignments", "1", "eds240-hw1-ccaswell25", "outputs", paste0("indoor_pollution_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".png")), res = 320, width = 12, height = 13, units = "in")
