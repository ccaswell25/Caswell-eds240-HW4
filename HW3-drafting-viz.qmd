...
I have one dataset from the Strava API and I will use the {rStrava} package to access this data successfully in R. After using the API to receive my data, I have been able to read-in the following variables related to my Strava activities:
id - unique identifier for the activity
name - named title of the activity
sport_type - type of activity 
start_date - the start date and time of the activity (in ymdhms format)
year - year of the start date of the activity
month - month of the start date of the activity
day - day of the start date of the activity
week_number - the week number in the year
total_miles - the total miles completed during the activity (if applicable)
elevation_gain_ft - the total elevation gain during the activity in feet (if applicable)
avg_speed_mi_hr - total average speed of activity in miles per hour (if applicable)
...
**6.Mock up your visualizations using code. We understand that you will continue to iterate on these into HW #4 (particularly after receiving feedback), but by the end of HW #3, you should:
 -use appropriate strategies to highlight / focus attention on a clear message -include appropriate text such as titles, captions, axis labels -experiment with colors and typefaces / fonts -create a presentable / aesthetically-pleasing theme (e.g. (re)move gridlines / legends as appropriate, adjust font sizes, etc.)**
```{r}
#Loading Libraries ---
library(rStrava)
library(tidyverse)
...
ggplot(filtered_strava_activities, aes(x = training_week, y = day)) +
  ...
  scale_fill_continuous(low = "#FEFE62", high = "#D35FB7", name = "Total Daily Miles Ran", breaks = c(5,10,15,20,25,30, 35, 40)) + #adding color, legend title, and legend breaks
...
```{r}
#Plot # 3 -  Spider Plot
subset_strava_activities <- 
  strava_activities %>% 
  filter(start_date_local > "2023-04-30") %>% 
  filter(sport_type == "Run" | sport_type == "TrailRun" | sport_type == "WeightTraining" | sport_type == "Hike") %>% 
  group_by(sport_type, month) %>% 
  summarise(count(sport_type))


ggplot(subset_strava_activities, aes(x = sport_type, y = Count)) +
  geom_polygon(fill = "skyblue", color = "blue", size = 1) +
  coord_polar(start = 0) +  # Make the chart circular
  theme_minimal() +  # Apply a minimal theme
  labs(title = "Strava Activities",
       x = NULL, y = NULL,
       caption = "Source: Strava Data") +
